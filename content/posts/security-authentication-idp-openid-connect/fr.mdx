---
title: "Fédération d'identité : sécuriser les flux OIDC, OAuth2 et SAML"
subtitle: "Forest Admin Server comme SP en amont et IdP en aval pour les agents."
summary: "Décomposition des flux OIDC, des bearer tokens OAuth2 et des assertions SAML lorsqu'une plateforme joue à la fois le rôle de SP et d'IdP."
visualId: "security-authentication-idp-openid-connect"
...
<ArticleDiagram visualId="security-authentication-idp-openid-connect" />
...
1. **Utilisateur vers Forest Admin Server** (la frontière « Es-tu vraiment Bob de la compta ? », via SAML, OAuth ou simplement mot de passe + 2FA)
...
Pour pimenter un peu le tout, **nous n'hébergions pas les agents**. Ils tournaient bien au chaud derrière les pare-feu d'entreprise de nos clients. Vous vous demandez sûrement : « Comment un serveur SaaS peut-il envoyer des commandes à un serveur caché derrière un firewall ? » La réponse magique : les Server-Sent Events (SSE). L'agent ouvre une connexion sortante et nous y poussons nos données. (Mais honnêtement, la joie et le traumatisme absolu de maintenir des connexions SSE actives à travers des milliers de pare-feu méritent [un article dédié à part entière](../posts/architecture-sse-agent-communication)).

Cela nous imposait un modèle à double rôle merveilleusement complexe vis-à-vis de l'utilisateur :

- Forest Admin Server devait agir comme **Service Provider (SP)** vis-à-vis des grands fournisseurs d'identité externes (SAML d'entreprise comme Okta/Azure AD, et OAuth2/OIDC comme Google).
- Mais en même temps, Forest Admin Server devait aussi se comporter en **Identity Provider (IdP)** vis-à-vis des agents. Le client (frontend) est déjà authentifié au niveau de notre serveur, mais, pour récupérer les données de l'application, le frontend a besoin d'un token à fournir directement à l'agent hébergé chez le client. L'agent accorde une confiance aveugle à ces artefacts d'identité émis par Forest.
...
<figure className="post-visual">
  <img
    ...
    alt="Vue d'ensemble de la fédération d'identité montrant Forest Admin Server comme SP en amont et IdP en aval."
    loading="lazy"
    decoding="async"
  />
  <figcaption>
    Topologie globale : fédération amont vers Forest, puis accès délégué aval de
...
<figure className="post-visual">
  ...
  />
  <figcaption>
    Détail du flux OIDC amont : redirections, callback code, échange de token et
...
<figure className="post-visual">
  <img
    ...
    alt="Flux de validation bearer aval où Forest émet un token et l'agent valide la signature et les claims avant allow ou deny."
    loading="lazy"
    decoding="async"
  />
  <figcaption>
    Chemin de contrôle aval : chaque vérification de claim est explicite et la
    décision est observable.
  </figcaption>
</figure>
...
<figure className="post-visual">
  <img
    ...
    alt="Pont SAML vers OAuth2 via RFC 7522 où Forest valide une assertion puis effectue un échange de token contraint."
    loading="lazy"
    decoding="async"
  />
  <figcaption>
    Pont RFC 7522 : validation stricte de l'assertion SAML amont avant échange et
    émission de l'accès aval.
  </figcaption>
</figure>
