...
3. **Step 3: The Enrichment (PostgreSQL)**
   Finally, before returning the payload to the client, we take the Elasticsearch results and aggregate/enrich them with the latest relational data from PostgreSQL (this was more due to another design issue we didn't fix in time).
