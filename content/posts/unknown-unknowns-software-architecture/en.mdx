...
The conceptual foundation came from Paul Austin Murphy's essay [Rumsfeld’s Logic of Known Knowns, Known Unknowns and Unknown Unknowns](https://medium.com/paul-austin-murphys-essays-on-philosophy/rumsfelds-logic-of-known-knowns-known-unknowns-and-unknown-unknowns-f506db31ac74). It takes the famous formulation seriously as a problem of logic and knowledge: some gaps are visible enough to investigate, while others sit outside the boundary of what we can currently describe.

But the article that genuinely stayed with me was Thariq Shihipar's [A Field Guide to Fable: Finding Your Unknowns](https://x.com/trq212/article/2073100352921215386). Thariq, who works at Anthropic, describes the gap between the **map** we give an AI system—our prompt, skills, context, and specification—and the **territory** where the work actually happens: the repository, product, users, operational constraints, and all the decisions nobody wrote down.

When the map does not contain something the territory requires, the agent has to guess. Stronger models make those guesses look increasingly complete, which is useful right up until the missing assumption is the one that matters.

That idea resonated far beyond AI-assisted coding. I kept thinking about architecture reviews, production incidents, and all the times a system behaved correctly according to every test we had written—then failed through an interaction that was absent from our model. The test suite was not lying. We had simply asked it yesterday's questions.

Unknown unknowns are not a mystical category of failure. They are ordinary system properties that become visible only when a boundary is crossed: more tenants, a different provider, a deploy during a traffic spike, a new data grain, a partial outage, or a new operator using a workflow in a way nobody expected.
...
## Why architecture diagrams create false confidence (production never signed them)
...
Boxes and arrows are remarkably well behaved. Retries, clocks, operators, and third-party APIs have apparently not received the memo.
...
## From an article to an operating habit at RockFi

I shared these articles with several teams at RockFi because they gave us better language for a problem we were already experiencing. Not as two more links destined for a polite thumbs-up before disappearing into Slack, but as a way to improve how we prepare, execute, and review work.

The useful change is to make unknown discovery part of the delivery loop:

- **Before implementation:** ask what the brief assumes, which parts of the territory have not been inspected, which owners or systems could change the design, and what would invalidate the current plan.
- **During implementation:** compare the real repository, data, APIs, and runtime behaviour with the original map. Newly discovered dependencies should update the plan instead of being quietly absorbed into the code.
- **After implementation:** use QA, logs, metrics, reconciliation, adversarial review, and production feedback to search for behaviours outside the expected path—not only to confirm that the expected path works.

AI is useful throughout this loop because it can inspect a wider surface, challenge assumptions, and generate targeted questions. It is not an oracle for unknown unknowns. It can only discover what leaves evidence in the territory it is allowed to inspect.

This has pushed us toward more explicit definitions of done, stronger QA passes, better feedback loops, and processes that preserve newly discovered constraints instead of letting them return as somebody else's production incident six months later.
...
<figure className="post-visual">
  <img
    ...
    alt="Architecture learning loop from a decision and implicit assumptions through real interaction, failure, evidence, and a revised contract."
  />
  <figcaption>
    Unexpected behaviour becomes useful when it produces observable evidence and a revised boundary.
  </figcaption>
</figure>
...
It should also remain a design review, not become a three-hour philosophical investigation into whether the database itself truly exists. Proportionality still applies.
...
Original inspiration: [Rumsfeld’s Logic of Known Knowns, Known Unknowns and Unknown Unknowns](https://medium.com/paul-austin-murphys-essays-on-philosophy/rumsfelds-logic-of-known-knowns-known-unknowns-and-unknown-unknowns-f506db31ac74) and [A Field Guide to Fable: Finding Your Unknowns](https://x.com/trq212/article/2073100352921215386).
