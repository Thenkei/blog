...
import contextEngineeringDiagram from "../../../src/assets/images/posts/context-engineering-en.svg";
...
This is the shift I keep seeing in real engineering work. Once code generation becomes cheap, the difficult part is no longer persuading the model to produce syntax. It is stopping the model from using the wrong repository convention, the superseded decision, or the configuration file that looks important only because it has an aggressively confident name.

## A prompt cannot compensate for missing state (it will still try)
...
It may now have 80,000 tokens of context and the confidence of someone who read the wrong README very carefully.
...
## Context budgets require prioritisation (everything is not a strategy)
...
<figure className="post-visual">
  <img
    ...
    alt="Context engineering flow from task constraints through relevance, authorization, freshness, and provenance checks to agent action and evidence."
  />
  <figcaption>
    Reliable agent execution depends on context that is relevant, authorized, fresh, and testable.
  </figcaption>
</figure>
...
“The model will probably understand” is not a policy. It is a sentence that tends to age badly once production data and write permissions enter the conversation.
