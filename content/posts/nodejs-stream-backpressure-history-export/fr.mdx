...
Ce pipeline d'export est littéralement né d'un traumatisme opérationnel : les utilisateurs pouvaient exiger des logs d'activité sur des plages de dates dantesques, parfois depuis le tout premier jour du projet. Les données dormaient dans Elasticsearch, chaque événement devait être farci d'infos provenant de bases relationnelles, puis le monstrueux CSV final devait atterrir sagement sur S3. Plutôt simple, non ?
...
- Partie 2 - [Jobify + BullMQ + contrats worker/queue dans NestJS](../jobify-workers-queues-nestjs/fr.mdx)
- Partie 3 - [durcissement idempotence + debounce (reschedule vs time-frame)](../idempotency-debounce-jobify-bullmq/fr.mdx)
...
- Continuer avec les contrats d'architecture worker/queue dans [Partie 2](../jobify-workers-queues-nestjs/fr.mdx).
- Puis durcir dedupe et tempetes de triggers dans [Partie 3](../idempotency-debounce-jobify-bullmq/fr.mdx).
