---
title: "Jobify au-dessus de BullMQ : un pattern de production pour jobs, workers et queues"
subtitle: "De l'intégration NestJS classique aux runners typés, au séquentiel et aux exports robustes."
summary: "Un pattern de production BullMQ/NestJS pour des workers plus fiables, traçables et prévisibles sous charge."
visualId: "jobify-workers-queues-nestjs"
...
**Série : Async Workloads at Scale (partie 2/3)**

Ce post fait le lien architectural entre les deux autres articles de la série :

- Partie 1 - [backpressure Node.js, streams et exports S3 multipart](../posts/nodejs-stream-backpressure-history-export)
- Partie 2 (courante) - Jobify sur BullMQ pour les contrats worker/queue.
- Partie 3 - [stratégies idempotence/debounce (reschedule vs time-frame)](../posts/idempotency-debounce-jobify-bullmq)

**NestJS est le point d'intégration framework. Jobify est le contrat d'exécution.**

## Intégration NestJS classique (point de départ)

<ArticleDiagram visualId="jobify-workers-queues-nestjs" />
...
- **Fonction nommée obligatoire :** clé de processor stable.
- **Registration seulement côté worker :** évite les doublons de handlers.
- **Priorité dynamique :** valeur statique, fonction sync ou async.
- **Contrat debounce strict :** `delayInMs` + `makeJobId` + `name`.

## Queue centralisée dans queue-service.ts

- propagation de trace au moment de l'enqueue ;
- reschedule des jobs delayed via `changeDelay` ;
- nettoyage des repeatables pour éviter les doubles schedules ;
- hygiène par défaut avec `removeOnComplete/removeOnFail`.

## Exécution worker standardisée dans worker-service.ts

Ordre des wrappers: monitoring -> onError -> timeout -> work.

Ce point est critique : même comportement de timeout, même policy d'erreur, mêmes métriques, quel que soit le job.

## Exécution séquentielle avec make-jobify-sequential

- **sequentialKey** definit la lane;
- **processingKey** definit le type d'action;
- **identifier** scope une entite.

Les arguments sont stockés dans Redis (liste), traités dans l'ordre et dépilés dans `finally` pour éviter le blocage d'une lane en cas d'échec.
...
- branches d'e-mails success/error directement branchées.
...
- **Suppression en file d'attente :** supprimer un environnement cloud implique d'effacer des objets AWS S3 et de mettre à jour le système central. On envoie tout cela dans un `cloudDeleteService.queuedDeleteAgent()`. Si AWS nous applique une limite de débit, BullMQ retentera la suppression plus tard, sans laisser d'infrastructure fantôme traîner (et sans enflammer nos factures NAT).
...
3. Ajouter des jobId déterministes pour l'idempotence et le debounce.
4. N'activer le séquentiel que là où l'ordre est obligatoire.
5. Mesurer queue depth, failures et p95/p99 avant généralisation.

## Règles de design de survie (À graver dans le marbre)

- ne pas exposer les objets BullMQ Job à la logique métier par défaut. Sinon, votre cœur pur sera corrompu par les entrailles de BullMQ, et la prochaine version majeure vous fera pleurer de sang ;
- imposer des timeouts par défaut pour garantir les shutdowns ;
- garder le contrat producer/consumer au même endroit ;
- traiter les lanes séquentielles comme une ressource rare.

Le principal gain n'est pas seulement le débit. C'est le déterminisme opérationnel sur l'ensemble des workflows asynchrones.

## Navigation dans la série

- Besoin du contexte stream/export en amont ? Lire [Partie 1](../posts/nodejs-stream-backpressure-history-export).
- Besoin des patterns idempotence/debounce ensuite ? Lire [Partie 3](../posts/idempotency-debounce-jobify-bullmq).
