...
summary: "Une panne survenue un week-end chez Forest Admin à cause de l'épuisement de la mémoire Redis, et les leçons qu'elle apporte sur le cycle de vie des données."
visualId: "redis-memory-exhaustion-post-mortem"
...
Stocker des objets complexes et volumineux dans Redis pour obtenir un service ultra-rapide semble être une excellente idée, jusqu'à ce que cela ne le soit plus. Chez Forest Admin, nous avons appris à nos dépens que traiter une seule instance Redis comme une solution universelle pouvait entraîner des pannes en cascade catastrophiques.

Voici l'histoire d'une panne survenue un week-end, causée par une erreur de mémoire insuffisante (OOM), et les leçons d'architecture et de cycle de vie des données que nous en avons tirées.

## Le contexte

<ArticleDiagram visualId="redis-memory-exhaustion-post-mortem" />
...
## Le syndrome de la grenouille dans l'eau chaude
...
<ArticleMedia mediaId="redis-memory-pressure" />
...
### 1. Pas d'alerte mémoire dans Datadog
...
### 2. Une architecture avec un point unique de défaillance (SPOF)
...
### 3. Des cycles de vie des données inefficaces
...
## Les corrections et leçons à retenir
...
- **Raccourci les TTL** : Les rendre beaucoup plus agressifs pour garantir que la mémoire soit libérée plus rapidement.
...
- **Compressé les payloads** : Un collègue ([Guillaume Gautreau](https://www.ghusse.com/redis/node/compress/does-it-worth-compressing-large-json-in-redis/)) a pris le temps d'étudier et d'implémenter une solution de compression des données _avant_ leur stockage dans Redis. Non seulement cela a réduit significativement notre consommation mémoire, mais cela s'est également traduit par des temps de transport HTTPS plus courts grâce à la taille réduite des payloads.
