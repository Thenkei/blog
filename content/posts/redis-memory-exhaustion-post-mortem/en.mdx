...
We initially thought we had data retention covered. We had a TTL (Time-To-Live) configured, but it simply wasn't aggressive enough. Furthermore, we realized that we weren't actively purging data belonging to deleted projects—leaving stale, bloated objects consuming precious RAM for absolutely no reason.
...
- **Compressed the Payloads**: A colleague ([Guillaume Gautreau](https://www.ghusse.com/redis/node/compress/does-it-worth-compressing-large-json-in-redis/)) took the time to study and implement a compression solution for the data _before_ it gets stored in Redis. Not only did this optimize our memory costs significantly, but it also resulted in faster HTTPS transport times due to the smaller payload sizes.
